...
function getAllItems($conn)
 ...
 function order($conn,$data,$product_id)
 {
 	$pen="pending";
 	$stmtchangeenum = $conn->prepare("UPDATE tbl_item set product_status=:product_status WHERE product_id=:product_id");
 	$stmtchangeenum->bindParam('product_id',$product_id);
 	$stmtchangeenum->bindParam('product_status',$pen);
 	$stmtchangeenum->execute();
 	$stmtOrder = $conn->prepare("INSERT INTO tbl_orders 
 								(`product_id`, `cust_name`, `cust_address`, `cust_phone`)
 								VALUES(:product_id, :cust_name, :cust_address, :cust_phone)");
    
    dd($data);
 	$stmtOrder->bindParam('product_id',$product_id);
 	$stmtOrder->bindParam('cust_name',$data['cust_name']);
 	$stmtOrder->bindParam('cust_address',$data['cust_address']);
 	$stmtOrder->bindParam('cust_phone',$data['cust_phone']);
