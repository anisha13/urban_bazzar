...
<?php include'layouts/header.php';
	...
	$oldCart=$conn->prepare("SELECT product_amount from tbl_cart where `product_id`=:product_id AND `cust_id`=:cust_id");
	$oldCart->bindParam(':product_id',$product_id);
	$oldCart->bindParam(':cust_id',$cust_id);

	$oldCart->execute();
	$oldCart=$oldCart->fetch();
	...
	if(isset($_POST['cartbtn']))
	{
		...
		{
			if(cart($conn,$_POST,$product_id,$cust_id,$oldCart["product_amount"]))
			{
			...
			<div class="w-size14 p-t-30 respon5">
				...
				<form method="POST" enctype="multipart/form-data">				
					<div class="flex-w bo5 of-hidden w-size17">
						<button class="btn-num-product-down color1 flex-c-m size7 bg8 eff2" onclick="son()">
							<i class="fs-12 fa fa-minus" aria-hidden="true"></i>
						</button>
						<input class="size8 m-text18 t-center num-product" type="number" id="second" name="amount" value="<?php 
									if(!isset($oldCart["product_amount"]))
									{
										$oldCart["product_amount"]=0;
										echo "1";
									}
									else
										echo "1"; ?>">
